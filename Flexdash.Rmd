...
-------------------------
```{r, echo=FALSE}
selectInput("index_type", "Choose Index Type:",
            choices = c("Equality Index" = "equality_cont",
                        "Legal Index" = "legal_cont",
                        "Public Opinion Index" = "po_cont"),
            selected = "equality_cont")
...
renderPlot({
  input$update
  isolate({
    selected_index <- input$index_type
    
    studyabroad$index_value <- studyabroad[[selected_index]]
    ...
    studyabroad |>
      ...
      filter(region_id == 1) |>
  ggplot(aes_string(x = "region", y = "selected_index")) +
  geom_col(fill = "skyblue") +
  coord_flip() +
      labs(x = "Continent", y = "Index Value", 
           title = paste("Average", gsub("_", " ", selected_index), "by Continent")) +
      theme_minimal()
  })
})
```
