...
studyabroad$save_share <- ifelse(studyabroad$save_share == "Save this program", "J-term", "Semester")
studyabroad$avg <- studyabroad |>
  group_by(region) |>
  mutate(avg_equality = mean(equality_index),
           avg_legal = mean(legal_index),
           avg_po = mean(public_opinion_index))
```

```{r}
#equality equality
equality <- clean_names(equality)
equality$public_opinion_index <- str_replace_all(equality$public_opinion_index, "â.", "NA")
